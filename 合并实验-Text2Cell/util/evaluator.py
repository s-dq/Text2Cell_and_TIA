...
class Evaluator(object):
    ...
    def summary(self, eval_mode = 'edge_rel | lloc'):
        ...
        if 'edge_rel' in eval_mode:
            ...
            print('需要合并，且被预测合并=', self.matched_fgRel)
            print('被预测为合并总数=', self.pred_fgRel_sum)
            print('不需要合并，被预测不需要合并', self.matched_bgRel)
            print('被预测不合并总数', self.pred_bgRel_sum)
            print('acc=', (self.matched_fgRel + self.matched_bgRel) / (self.pred_fgRel_sum + self.pred_bgRel_sum))
            select_metric += fg_f1
