...
class Res2TimModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        parser.add_argument('--rel_classes', type=int, default=2, help='the number of relationships for classification')
    ...
    def set_input(self, input):
        ...
        def encode_texts(texts):
            ...
            sentence_vectors = outputs.pooler_output
            return sentence_vectors

        with jsonlines.open('./datasets/icdar13table/vision.jsonl') as reader:
            vision = list(reader)
        texts = []
        for i in range(len(vision)):
            if vision[i]['table_id'] == input['tb_names'][0].split('_')[0]:
                texts = vision[i]['text']
        tensor_ones = encode_texts(texts)
        tensor_combined = torch.cat((input['spatial_feats'], tensor_ones), dim=1)
        self.spatial_feats = tensor_combined
    ...
    def backward(self):
        self.weights = self.weights * 10000
